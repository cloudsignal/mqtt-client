...
/**
 * Strip the `$share/<group>/` prefix from a subscription filter, leaving the
 * bare topic filter the broker will deliver against. MQTT 5 §4.8.2:
 * shared subscriptions deliver messages with the bare topic, not the share
 * group, so the client-side dispatch must match against the bare filter.
 */
const SHARED_SUB_PREFIX = /^\$share\/[^/]+\//
function bareSubscriptionFilter(filter: string): string {
  return filter.replace(SHARED_SUB_PREFIX, '')
}

/**
 * MQTT topic-filter match: returns true iff `topic` (the delivered topic,
 * always concrete with no wildcards) matches `filter` (which may contain
 * `+` single-level or `#` multi-level wildcards). MQTT 5 §4.7.
 *
 * No special handling for `$`-prefixed topics — the broker already gates
 * those at SUBACK time; client-side dispatch should still deliver them.
 */
function matchTopicFilter(filter: string, topic: string): boolean {
...
export class CloudSignalClient {
  ...
  _handleMessage(topic, message, packet) {
    ...
    // Dispatch by topic-filter match, not by literal subscription-string
    // equality. The Map is keyed by the full subscribe filter (including any
    // `$share/<group>/` prefix and `+` / `#` wildcards) for unsubscribe identity,
    // but the broker delivers messages with the BARE topic — so we strip the
    // share prefix and run an MQTT topic-filter match against the delivered
    // topic. Fast path: O(1) literal equality first; fall back to iteration
    // only for shared / wildcard subscriptions.
    let dispatchedToSubscription = false
    const direct = this.ongoingSubscriptions.get(topic)
    if (direct && direct.callback) {
      direct.callback(topic, message.toString(), packet)
      dispatchedToSubscription = true
    } else {
      for (const [filter, sub] of this.ongoingSubscriptions) {
        if (!sub.callback) continue
        if (filter === topic) continue // already handled by the fast path
        if (matchTopicFilter(bareSubscriptionFilter(filter), topic)) {
          sub.callback(topic, message.toString(), packet)
          dispatchedToSubscription = true
          // MQTT spec: a single message may match multiple filters; deliver
          // to all matching callbacks (continue the loop).
        }
      }
    }

    // Defensive: if no per-topic callback claimed this message AND no
    // global callback is registered, log at debug so any future routing
    // regression surfaces instead of silently dropping the message.
    if (!dispatchedToSubscription && this._messageCallbacks.length === 0) {
      this.logger.debug(`Message on ${topic} had no matching subscription callback or global handler`)
    }
